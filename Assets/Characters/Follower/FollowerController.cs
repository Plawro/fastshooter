...
public class FollowerController : MonoBehaviour
{
    ...
    // Character has no gravity + animations based on velocity are only on predict mode
    ...
    void Patrol()
    {
        ...
        if (!agent.pathPending && agent.remainingDistance <= 0.5f)
        {
            ...
            if (current >= points.Length)
            {
                ...
                isPatrolling = false;
                StartIdle();
                Debug.Log("Patrol complete, starting idle");
            }
            else
            {
                agent.SetDestination(points[current].position);
                Debug.Log("Patrolling to point " + current);
            }
        }
    }
